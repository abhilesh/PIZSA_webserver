	...
	for interface in interface_residues:
		...
		for res_pair in interface_residues[interface]:
			for interaction in interface_residues[interface][res_pair]:
				...
				if hbond:
					overlap = (vdW_radii_dict[res_key_1] + vdW_radii_dict[res_key_2]) - inter_dist - allowance

				else:
					overlap = (vdW_radii_dict[res_key_1] + vdW_radii_dict[res_key_2]) - inter_dist

				if overlap > cut_off:
					if res_pair in clashes[interface].keys():
						clashes[interface][res_pair].append(overlap)
					else:
						clashes[interface].update({res_pair : [overlap]})
